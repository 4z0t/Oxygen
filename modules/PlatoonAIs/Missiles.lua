...
--[[
    ...
    UI_Lua local makeThread = function(f, ...) local thread = ForkThread(function() f(SuspendCurrentThread()) end ) coroutine.resume(thread, unpack(arg)) return thread end local t = function(...) reprsl(arg) LOG (coroutine.yield(1,2,3)) LOG (coroutine.yield('f',5,6)) end  local th = makeThread(t) print(coroutine.resume(th, 'a', 'b', 'c')) print(coroutine.resume(th, 'b')) print(coroutine.resume(th, 'c'))
    ]]